...
local GenerateEntity = {}
GenerateEntity.__index = GenerateEntity


function GenerateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GenerateEntity)
  self._name = "generate"
  self._client = client
...
function GenerateEntity:get_name()
  return self._name
end


function GenerateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GenerateEntity.new(self._client, opts)
end
...
function GenerateEntity:mark_deleted()
  self._deleted = true
end


function GenerateEntity:deleted()
  return true == self._deleted
end


function GenerateEntity:data_set(args)
  if args ~= nil then
...
function GenerateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GenerateEntity:match_set(args)
  if args ~= nil then
...
function GenerateEntity:match_get()
...
function GenerateEntity:stream(action, args, callopts)
...
---@param reqmatch GenerateLoadMatch
---@param ctrl? table
---@return Generate
---@return string? err
function GenerateEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GenerateEntity:_run_op(ctx, post_done)
  local utility = self._utility
