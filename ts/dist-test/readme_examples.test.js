...
const __1 = require("..");
const SDK_NAME = 'EmailValidationApi2SDK';
...
const TEST_SEED = { "entity": { "dns_result": { "test01": { "id": "test01" } }, "domain": { "test01": { "id": "test01" } }, "email_validate": { "test01": { "id": "test01" } }, "generate": { "test01": { "id": "test01" } }, "grammar": { "test01": { "id": "test01" } }, "ipn": { "test01": { "id": "test01" } }, "redact": { "test01": { "id": "test01" } }, "ssl": { "test01": { "id": "test01" } }, "utility": { "test01": { "id": "test01" } }, "whoi": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+EmailValidationApi2SDK\b/.test(code) ||
        /\bEmailValidationApi2SDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+EmailValidationApi2SDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/EmailValidationApi2SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
