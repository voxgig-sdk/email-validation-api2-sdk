# EmailValidationApi2 SDK test runner
...
class EmailValidationApi2TestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            EmailValidationApi2TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = EmailValidationApi2TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = EmailValidationApi2TestRunner.getenv("EMAIL_VALIDATION_API2_TEST_LIVE")
        override = EmailValidationApi2TestRunner.getenv("EMAIL_VALIDATION_API2_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = EmailValidationApi2TestRunner.getenv(key)
        ...
        explain = EmailValidationApi2TestRunner.getenv("EMAIL_VALIDATION_API2_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if EmailValidationApi2TestRunner._test_control is not None:
            return EmailValidationApi2TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                EmailValidationApi2TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            EmailValidationApi2TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = EmailValidationApi2TestRunner.load_test_control()
...
def load_env_local():
    EmailValidationApi2TestRunner.load_env_local()


def env_override(m):
    return EmailValidationApi2TestRunner.env_override(m)


def entity_data(v):
    return EmailValidationApi2TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return EmailValidationApi2TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return EmailValidationApi2TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return EmailValidationApi2TestRunner.load_test_control()


def live_client_options():
    return EmailValidationApi2TestRunner.live_client_options()


def live_delay_ms():
    return EmailValidationApi2TestRunner.live_delay_ms()
