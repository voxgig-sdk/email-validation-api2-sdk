...
local IpnEntity = {}
IpnEntity.__index = IpnEntity


function IpnEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IpnEntity)
  self._name = "ipn"
  self._client = client
...
function IpnEntity:get_name()
  return self._name
end


function IpnEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IpnEntity.new(self._client, opts)
end
...
function IpnEntity:mark_deleted()
  self._deleted = true
end


function IpnEntity:deleted()
  return true == self._deleted
end


function IpnEntity:data_set(args)
  if args ~= nil then
...
function IpnEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IpnEntity:match_set(args)
  if args ~= nil then
...
function IpnEntity:stream(action, args, callopts)
...
---@param reqmatch IpnLoadMatch
---@param ctrl? table
---@return Ipn
---@return string? err
function IpnEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function IpnEntity:_run_op(ctx, post_done)
  local utility = self._utility
