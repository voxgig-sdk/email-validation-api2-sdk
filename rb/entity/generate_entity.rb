...
class GenerateEntity
  ...
  def make
    opts = @_entopts.dup
    GenerateEntity.new(@_client, opts)
  end
  ...
  # @return [Generate, Hash] the current Generate data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Generate fields)
  def match_get
  ...
  #
  # @param reqmatch [GenerateLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Generate.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Generate, Hash] the loaded Generate; raises EmailValidationApi2Error on failure
