...
class TestUtilityDirect:

    def test_should_direct_load_utility(self):
        setup = _utility_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-utility", "live" if setup["live"] else "unit")
        if _skip:
...
def _utility_direct_setup(mockres):
