...
// EmailValidationApi2 SDK feature test
//
...
class FtCtrl extends EmailValidationApi2Control
{
...
class FtClient
{
    ...
    private EmailValidationApi2Utility $utility;

    public function __construct(EmailValidationApi2Utility $utility, array $options)
    {
    ...
    public function get_utility(): EmailValidationApi2Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EmailValidationApi2Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EmailValidationApi2Context $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (EmailValidationApi2Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EmailValidationApi2Config::make_feature($name);
    ...
    private static function buildUrl(EmailValidationApi2Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EmailValidationApi2Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EmailValidationApi2Response($fetched);
        }
    ...
    private function fail(EmailValidationApi2Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EmailValidationApi2Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
