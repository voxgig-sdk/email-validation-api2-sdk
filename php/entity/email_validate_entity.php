...
// EmailValidationApi2 SDK EmailValidate entity
...
class EmailValidateEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new EmailValidateEntity($this->_client, $opts);
    }

    /**
     * @param EmailValidate|array $args EmailValidate data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = EmailValidationApi2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return EmailValidate|array The current EmailValidate data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of EmailValidate fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = EmailValidationApi2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of EmailValidate fields).
     */
    ...
    /**
     * Load a single EmailValidate.
     *
     * @param EmailValidateLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed EmailValidateLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return EmailValidate|array The loaded EmailValidate as an assoc-array at the
     *   SDK boundary; throws EmailValidationApi2Error on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = EmailValidationApi2Helpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
