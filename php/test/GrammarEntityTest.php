...
class GrammarEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = EmailValidationApi2SDK::test(null, null);
        $ent = $testsdk->Grammar(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = grammar_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "grammar." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set EMAIL_VALIDATION_API2_TEST_GRAMMAR_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $grammar_ref01_ent = $client->Grammar(null);
...
function grammar_basic_setup($extra)
{
    Runner::load_env_local();

    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/grammar/GrammarTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["grammar01", "grammar02", "grammar03"] as $k) {
    ...
    $entid_env_raw = getenv("EMAIL_VALIDATION_API2_TEST_GRAMMAR_ENTID");
