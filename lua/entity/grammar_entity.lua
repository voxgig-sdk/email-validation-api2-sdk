...
local GrammarEntity = {}
GrammarEntity.__index = GrammarEntity


function GrammarEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GrammarEntity)
  self._name = "grammar"
  self._client = client
...
function GrammarEntity:get_name()
  return self._name
end


function GrammarEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GrammarEntity.new(self._client, opts)
end
...
function GrammarEntity:mark_deleted()
  self._deleted = true
end


function GrammarEntity:deleted()
  return true == self._deleted
end


function GrammarEntity:data_set(args)
  if args ~= nil then
...
function GrammarEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GrammarEntity:match_set(args)
  if args ~= nil then
...
function GrammarEntity:match_get()
...
function GrammarEntity:stream(action, args, callopts)
...
---@param reqdata GrammarCreateData
---@param ctrl? table
---@return Grammar
---@return string? err
function GrammarEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function GrammarEntity:_run_op(ctx, post_done)
  local utility = self._utility
