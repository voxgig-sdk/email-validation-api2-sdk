...
describe("IpnEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Ipn(nil)
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set EMAIL_VALIDATION_API2_TEST_IPN_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local ipn_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.ipn")))
    local ipn_ref01_data = nil
...
function ipn_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/ipn/IpnTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read ipn test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("EMAIL_VALIDATION_API2_TEST_IPN_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["EMAIL_VALIDATION_API2_TEST_IPN_ENTID"])
