...
func TestGenerateEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set EMAIL_VALIDATION_API2_TEST_GENERATE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		generateRef01MatchDt0 := map[string]any{}
		generateRef01DataDt0Loaded, err := generateRef01Ent.Load(generateRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		if generateRef01DataDt0Loaded == nil {
...
func generateBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("EMAIL_VALIDATION_API2_TEST_GENERATE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["EMAIL_VALIDATION_API2_TEST_GENERATE_ENTID"])
