...
func TestSslEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Ssl(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Ssl(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		sslRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.ssl")))
		...
		sslRef01Match := map[string]any{}

		sslRef01ListResult, err := sslRef01Ent.List(sslRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, sslRef01ListOk := sslRef01ListResult.([]any)
		if !sslRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", sslRef01ListResult)
		}
...
func sslBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("EMAIL_VALIDATION_API2_TEST_SSL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["EMAIL_VALIDATION_API2_TEST_SSL_ENTID"])
