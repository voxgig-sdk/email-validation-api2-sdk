...
local DnsResultEntity = {}
DnsResultEntity.__index = DnsResultEntity


function DnsResultEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DnsResultEntity)
  self._name = "dns_result"
  self._client = client
...
function DnsResultEntity:get_name()
  return self._name
end


function DnsResultEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DnsResultEntity.new(self._client, opts)
end
...
function DnsResultEntity:mark_deleted()
  self._deleted = true
end


function DnsResultEntity:deleted()
  return true == self._deleted
end


function DnsResultEntity:data_set(args)
  if args ~= nil then
...
function DnsResultEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DnsResultEntity:match_set(args)
  if args ~= nil then
...
function DnsResultEntity:match_get()
...
function DnsResultEntity:stream(action, args, callopts)
...
---@param reqmatch DnsResultLoadMatch
---@param ctrl? table
---@return DnsResult
---@return string? err
function DnsResultEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DnsResultEntity:_run_op(ctx, post_done)
  local utility = self._utility
