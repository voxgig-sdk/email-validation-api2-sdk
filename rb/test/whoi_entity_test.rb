...
class WhoiEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = EmailValidationApi2SDK.test(seed, nil)
    seen = base.Whoi(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Whoi(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set EMAIL_VALIDATION_API2_TEST_WHOI_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    whoi_ref01_list_result = whoi_ref01_ent.list(whoi_ref01_match, nil)
    assert whoi_ref01_list_result.is_a?(Array)
...
def whoi_basic_setup(extra)
  ...
  entid_env_raw = ENV["EMAIL_VALIDATION_API2_TEST_WHOI_ENTID"]
