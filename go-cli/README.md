# email-validation-api2-cli

boru-driven command-line client **and** interactive REPL for the EmailValidationApi2
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/email-validation-api2-cli)
make build

# 2. See usage (words, entities, env vars)
./email-validation-api2-cli --help

# 3. Provide credentials once, via the environment
export EMAIL_VALIDATION_API2_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./email-validation-api2-cli load 1 dns_result            # {id:1} shorthand
./email-validation-api2-cli load '{id:1}' dns_result       # explicit match map

# 5. Override the API base URL for a single call
EMAIL_VALIDATION_API2_BASE=https://api.example.com ./email-validation-api2-cli load 1 dns_result

# 6. No arguments -> interactive REPL
./email-validation-api2-cli
email-validation-api2> load 1 dns_result
email-validation-api2> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/email-validation-api2-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/email-validation-api2-cli load 1 dns_result
   ```
...
```sh
./email-validation-api2-cli load 1 dns_result          # scalar shorthand for {id:1}
./email-validation-api2-cli load '{id:1}' dns_result     # explicit match map
```
...
./email-validation-api2-cli load 1 dns_result
```
...
Run with no arguments to open a REPL (prompt `email-validation-api2>`). Each line is
evaluated as its own boru expression:

```text
$ ./email-validation-api2-cli
email-validation-api2> load 1 dns_result
email-validation-api2> /help
email-validation-api2> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `dns_result`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/email-validation-api2-cli`. |
...
(`./email-validation-api2-cli <expr>`) and interactively (the REPL), and expressions compose the
