...
func TestDomainDirect(t *testing.T) {
	t.Run("direct-list-domain", func(t *testing.T) {
		setup := domainDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-domain", _mode); _shouldSkip {
			if _reason == "" {
...
func domainDirectSetup(mockres any) *domainDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"EMAIL_VALIDATION_API2_TEST_DOMAIN_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["EMAIL_VALIDATION_API2_TEST_DOMAIN_ENTID"]; ok {
