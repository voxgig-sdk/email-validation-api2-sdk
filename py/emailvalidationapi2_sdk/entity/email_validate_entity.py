...
from emailvalidationapi2_sdk.emailvalidationapi2_types import (
    EmailValidate,
...
class EmailValidateEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return EmailValidateEntity(self._client, opts)
    ...
    def data_get(self) -> EmailValidate:
    ...
    def match_get(self) -> EmailValidate:
    ...
    def load(self, reqmatch=None, ctrl=None) -> EmailValidate:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.EmailValidate().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
