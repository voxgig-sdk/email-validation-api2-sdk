...
class DnsResultEntity
  ...
  def make
    opts = @_entopts.dup
    DnsResultEntity.new(@_client, opts)
  end
  ...
  # @return [DnsResult, Hash] the current DnsResult data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of DnsResult fields)
  def match_get
  ...
  #
  # @param reqmatch [DnsResultLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.DnsResult.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [DnsResult, Hash] the loaded DnsResult; raises EmailValidationApi2Error on failure
