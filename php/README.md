# EmailValidationApi2 PHP SDK



The PHP SDK for the EmailValidationApi2 API — an entity-oriented client using PHP conventions.
...
```php
<?php
require_once 'emailvalidationapi2_sdk.php';

$client = new EmailValidationApi2SDK([
    "apikey" => getenv("EMAIL_VALIDATION_API2_APIKEY"),
]);
```
...
```php
$client = EmailValidationApi2SDK::test();

// Entity ops return the ENTITY (throws on error);
// call data_get() for the mock record.
$utility = $client->Utility()->load(["input" => "example"]);
print_r($utility->data_get());
```
...
```php
require_once 'emailvalidationapi2_sdk.php';
$client = new EmailValidationApi2SDK($options);
```
...
```php
$client = EmailValidationApi2SDK::test($testopts, $sdkopts);
```
...
```
php/
├── emailvalidationapi2_sdk.php          -- Main SDK class
...
The main class (`emailvalidationapi2_sdk.php`) exports the SDK class
