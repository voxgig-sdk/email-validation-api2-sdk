...
class EmailValidationApi2MakeRequest
{
    public static function call(EmailValidationApi2Context $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new EmailValidationApi2Response(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new EmailValidationApi2Response($fetched);
        } else {
