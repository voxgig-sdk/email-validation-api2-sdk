...
local EmailValidateEntity = {}
EmailValidateEntity.__index = EmailValidateEntity


function EmailValidateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EmailValidateEntity)
  self._name = "email_validate"
  self._client = client
...
function EmailValidateEntity:get_name()
  return self._name
end


function EmailValidateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EmailValidateEntity.new(self._client, opts)
end
...
function EmailValidateEntity:mark_deleted()
  self._deleted = true
end


function EmailValidateEntity:deleted()
  return true == self._deleted
end


function EmailValidateEntity:data_set(args)
  if args ~= nil then
...
function EmailValidateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EmailValidateEntity:match_set(args)
  if args ~= nil then
...
function EmailValidateEntity:match_get()
...
function EmailValidateEntity:stream(action, args, callopts)
...
---@param reqmatch EmailValidateLoadMatch
---@param ctrl? table
---@return EmailValidate
---@return string? err
function EmailValidateEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function EmailValidateEntity:_run_op(ctx, post_done)
  local utility = self._utility
