...
describe("SslEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Ssl(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Ssl(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set EMAIL_VALIDATION_API2_TEST_SSL_ENTID JSON to run live")
      return
    end
    ...
    local ssl_ref01_match = {}

    local ssl_ref01_list_result, err = ssl_ref01_ent:list(ssl_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(ssl_ref01_list_result)
...
function ssl_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("EMAIL_VALIDATION_API2_TEST_SSL_ENTID")
