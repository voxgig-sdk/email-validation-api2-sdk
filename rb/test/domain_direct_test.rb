...
class DomainDirectTest < Minitest::Test
  def test_direct_list_domain
    setup = domain_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-domain", setup[:live] ? "live" : "unit")
    if _should_skip
...
def domain_direct_setup(mockres)
