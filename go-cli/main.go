// Command EmailValidationApi2-cli is a CLI + REPL for the EmailValidationApi2 SDK,
...
const prompt = "email-validation-api2"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "dns_result domain email_validate generate grammar ipn redact ssl utility whoi"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      EMAIL_VALIDATION_API2_APIKEY (api key), EMAIL_VALIDATION_API2_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: EMAIL_VALIDATION_API2_APIKEY carries the API key and
	// EMAIL_VALIDATION_API2_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("EMAIL_VALIDATION_API2_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("EMAIL_VALIDATION_API2_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewEmailValidationApi2SDK(opts)
