...
local RedactEntity = {}
RedactEntity.__index = RedactEntity


function RedactEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RedactEntity)
  self._name = "redact"
  self._client = client
...
function RedactEntity:get_name()
  return self._name
end


function RedactEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RedactEntity.new(self._client, opts)
end
...
function RedactEntity:mark_deleted()
  self._deleted = true
end


function RedactEntity:deleted()
  return true == self._deleted
end


function RedactEntity:data_set(args)
  if args ~= nil then
...
function RedactEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RedactEntity:match_set(args)
  if args ~= nil then
...
function RedactEntity:match_get()
...
function RedactEntity:stream(action, args, callopts)
...
---@param reqdata RedactCreateData
---@param ctrl? table
---@return Redact
---@return string? err
function RedactEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function RedactEntity:_run_op(ctx, post_done)
  local utility = self._utility
