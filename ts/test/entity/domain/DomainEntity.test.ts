...
describe('DomainEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set EMAIL_VALIDATION_API__TEST_DOMAIN_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['EMAIL_VALIDATION_API__TEST_DOMAIN_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'EMAIL_VALIDATION_API__TEST_DOMAIN_ENTID': idmap,
    'EMAIL_VALIDATION_API__TEST_LIVE': 'FALSE',
    'EMAIL_VALIDATION_API__TEST_EXPLAIN': 'FALSE',
    'EMAIL_VALIDATION_API__APIKEY': 'NONE',
  })

  idmap = env['EMAIL_VALIDATION_API__TEST_DOMAIN_ENTID']
