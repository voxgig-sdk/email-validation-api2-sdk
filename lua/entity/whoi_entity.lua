...
local WhoiEntity = {}
WhoiEntity.__index = WhoiEntity


function WhoiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WhoiEntity)
  self._name = "whoi"
  self._client = client
...
function WhoiEntity:get_name()
  return self._name
end


function WhoiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WhoiEntity.new(self._client, opts)
end
...
function WhoiEntity:mark_deleted()
  self._deleted = true
end


function WhoiEntity:deleted()
  return true == self._deleted
end


function WhoiEntity:data_set(args)
  if args ~= nil then
...
function WhoiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WhoiEntity:match_set(args)
  if args ~= nil then
...
function WhoiEntity:stream(action, args, callopts)
...
---@param reqmatch WhoiListMatch
---@param ctrl? table
---@return Whoi[]
---@return string? err
function WhoiEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function WhoiEntity:_run_op(ctx, post_done)
  local utility = self._utility
