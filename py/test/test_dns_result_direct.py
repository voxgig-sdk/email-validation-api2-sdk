...
class TestDnsResultDirect:

    def test_should_direct_load_dns_result(self):
        setup = _dns_result_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-dns_result", "live" if setup["live"] else "unit")
        if _skip:
...
def _dns_result_direct_setup(mockres):
