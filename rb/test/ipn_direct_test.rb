...
class IpnDirectTest < Minitest::Test
  def test_direct_load_ipn
    setup = ipn_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-ipn", setup[:live] ? "live" : "unit")
    if _should_skip
