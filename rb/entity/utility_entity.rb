...
class UtilityEntity
  ...
  def make
    opts = @_entopts.dup
    UtilityEntity.new(@_client, opts)
  end
  ...
  # @return [Utility, Hash] the current Utility data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Utility fields)
  def match_get
  ...
  #
  # @param reqmatch [UtilityLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Utility.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Utility, Hash] the loaded Utility; raises EmailValidationApi2Error on failure
