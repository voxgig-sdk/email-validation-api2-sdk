...
local UtilityEntity = {}
UtilityEntity.__index = UtilityEntity


function UtilityEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UtilityEntity)
  self._name = "utility"
  self._client = client
...
function UtilityEntity:get_name()
  return self._name
end


function UtilityEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UtilityEntity.new(self._client, opts)
end
...
function UtilityEntity:mark_deleted()
  self._deleted = true
end


function UtilityEntity:deleted()
  return true == self._deleted
end


function UtilityEntity:data_set(args)
  if args ~= nil then
...
function UtilityEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UtilityEntity:match_set(args)
  if args ~= nil then
...
function UtilityEntity:match_get()
...
function UtilityEntity:stream(action, args, callopts)
...
function UtilityEntity:_run_op(ctx, post_done)
  local utility = self._utility
