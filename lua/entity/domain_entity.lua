...
local DomainEntity = {}
DomainEntity.__index = DomainEntity


function DomainEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DomainEntity)
  self._name = "domain"
  self._client = client
...
function DomainEntity:get_name()
  return self._name
end


function DomainEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DomainEntity.new(self._client, opts)
end
...
function DomainEntity:mark_deleted()
  self._deleted = true
end


function DomainEntity:deleted()
  return true == self._deleted
end


function DomainEntity:data_set(args)
  if args ~= nil then
...
function DomainEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DomainEntity:match_set(args)
  if args ~= nil then
...
function DomainEntity:match_get()
...
function DomainEntity:stream(action, args, callopts)
...
function DomainEntity:_run_op(ctx, post_done)
  local utility = self._utility
