...
local SslEntity = {}
SslEntity.__index = SslEntity


function SslEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SslEntity)
  self._name = "ssl"
  self._client = client
...
function SslEntity:get_name()
  return self._name
end


function SslEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SslEntity.new(self._client, opts)
end
...
function SslEntity:mark_deleted()
  self._deleted = true
end


function SslEntity:deleted()
  return true == self._deleted
end


function SslEntity:data_set(args)
  if args ~= nil then
...
function SslEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SslEntity:match_set(args)
  if args ~= nil then
...
function SslEntity:stream(action, args, callopts)
...
---@param reqmatch SslListMatch
---@param ctrl? table
---@return Ssl[]
---@return string? err
function SslEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function SslEntity:_run_op(ctx, post_done)
  local utility = self._utility
