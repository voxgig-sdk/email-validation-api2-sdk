...
import { EmailValidationApi2SDK, BaseFeature, stdutil } from '../../..'
...
describe('EmailValidateEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'email_validate.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set EMAIL_VALIDATION_API__TEST_EMAIL_VALIDATE_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  let client = EmailValidationApi2SDK.test(options, extra)
  ...
  const idmapEnvVal = process.env['EMAIL_VALIDATION_API__TEST_EMAIL_VALIDATE_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'EMAIL_VALIDATION_API__TEST_EMAIL_VALIDATE_ENTID': idmap,
    'EMAIL_VALIDATION_API__TEST_LIVE': 'FALSE',
    'EMAIL_VALIDATION_API__TEST_EXPLAIN': 'FALSE',
    'EMAIL_VALIDATION_API__APIKEY': 'NONE',
  })

  idmap = env['EMAIL_VALIDATION_API__TEST_EMAIL_VALIDATE_ENTID']
