...
type Args struct {
	Entity string         `json:"entity" jsonschema:"dns_result | domain | email_validate | generate | grammar | ipn | redact | ssl | utility | whoi"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.EmailValidationApi2SDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "email-validation-api2_list",
		Description: "List records from EmailValidationApi2. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "email-validation-api2_load",
		Description: "Load a single record from EmailValidationApi2. " +
...
func runOp(client *sdk.EmailValidationApi2SDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.EmailValidationApi2SDK, name string) (sdk.EmailValidationApi2Entity, error) {
	switch strings.ToLower(name) {
	case "dns_result":
		return client.DnsResult(nil), nil
	case "domain":
		return client.Domain(nil), nil
	case "email_validate":
		return client.EmailValidate(nil), nil
	case "generate":
		return client.Generate(nil), nil
	case "grammar":
		return client.Grammar(nil), nil
	case "ipn":
		return client.Ipn(nil), nil
	case "redact":
		return client.Redact(nil), nil
	case "ssl":
		return client.Ssl(nil), nil
	case "utility":
		return client.Utility(nil), nil
	case "whoi":
