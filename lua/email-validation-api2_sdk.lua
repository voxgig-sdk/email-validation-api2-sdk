...
require("email-validation-api2_types")
...
local EmailValidationApi2SDK = {}
EmailValidationApi2SDK.__index = EmailValidationApi2SDK
...
EmailValidationApi2SDK._make_feature = _make_feature


function EmailValidationApi2SDK.new(options)
  local self = setmetatable({}, EmailValidationApi2SDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function EmailValidationApi2SDK:options_map()
...
function EmailValidationApi2SDK:get_utility()
  return Utility.copy(self._utility)
end


function EmailValidationApi2SDK:get_root_ctx()
  return self._rootctx
end


function EmailValidationApi2SDK:prepare(fetchargs)
...
function EmailValidationApi2SDK:direct(fetchargs)
...
function EmailValidationApi2SDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function EmailValidationApi2SDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "EmailValidationApi2SDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function EmailValidationApi2SDK:_raw_request(fetchargs)
  local utility = self._utility
...
function EmailValidationApi2SDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "EmailValidationApi2SDK: graphql: " .. msg
    res.graphql = errors
  end
...
function EmailValidationApi2SDK:DnsResult(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationApi2SDK:Domain(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationApi2SDK:EmailValidate(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationApi2SDK:Generate(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationApi2SDK:Grammar(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationApi2SDK:Ipn(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationApi2SDK:Redact(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationApi2SDK:Ssl(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationApi2SDK:Utility(data)
  local EntityMod = require("entity.utility_entity")
  if data == nil then
    if self._utility_ == nil then
      self._utility_ = EntityMod.new(self, nil)
    end
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationApi2SDK:Whoi(data)
...
function EmailValidationApi2SDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = EmailValidationApi2SDK.new(sdkopts)
  sdk.mode = "test"
