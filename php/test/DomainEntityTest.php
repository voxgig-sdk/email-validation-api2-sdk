...
class DomainEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = EmailValidationApi2SDK::test($seed, null);
        $seen = iterator_to_array($base->Domain(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = EmailValidationApi2Config::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = EmailValidationApi2SDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Domain(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set EMAIL_VALIDATION_API2_TEST_DOMAIN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function domain_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("EMAIL_VALIDATION_API2_TEST_DOMAIN_ENTID");
