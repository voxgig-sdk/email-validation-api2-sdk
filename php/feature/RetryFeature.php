...
// EmailValidationApi2 SDK retry feature
...
class EmailValidationApi2RetryFeature extends EmailValidationApi2BaseFeature
{
    ...
    public function init(EmailValidationApi2Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (EmailValidationApi2Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(EmailValidationApi2Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(EmailValidationApi2Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
