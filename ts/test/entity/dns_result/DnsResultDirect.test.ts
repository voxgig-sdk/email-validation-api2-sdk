...
describe('DnsResultDirect', async () => {
  ...
  test('direct-load-dns_result', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-dns_result', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['EMAIL_VALIDATION_API2_TEST_DNS_RESULT_ENTID']
