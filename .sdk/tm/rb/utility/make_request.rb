...
module EmailValidationApi2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = EmailValidationApi2Response.new({})
    ...
    if fetch_err
      ...
      response = EmailValidationApi2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EmailValidationApi2Response.new(fetched)
    else
