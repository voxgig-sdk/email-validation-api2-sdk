...
import (
	...
	omni "github.com/voxgig-sdk/email-validation-api2-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.EmailValidationApi2Error); is {
				out := map[string]any{
					"name":    "EmailValidationApi2Error",
...
func omniCtx(arg any, client *sdk.EmailValidationApi2SDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.EmailValidationApi2SDK); is {
			client = live
