# email-validation-api2-mcp

[MCP](https://modelcontextprotocol.io) server exposing the EmailValidationApi2 SDK as
two agent tools — `email-validation-api2_list` and `email-validation-api2_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/email-validation-api2-mcp)
make build

# 2. Provide credentials via the environment
export EMAIL_VALIDATION_API2_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user email-validation-api2 \
  -- /absolute/path/to/email-validation-api2-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./email-validation-api2-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// email-validation-api2_list: first page of records
{ "entity": "domain" }
{ "entity": "domain", "query": { } }

// email-validation-api2_load: one record by id
{ "entity": "dns_result", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/email-validation-api2-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user email-validation-api2 \
     -- "$PWD"/dist/*/email-validation-api2-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `email-validation-api2_list` and `email-validation-api2_load` tools now appear
   in new sessions. Ask the agent to *"list domain using email-validation-api2"*
   and it calls `email-validation-api2_list` with `{"entity":"domain"}`.
...
```sh
export EMAIL_VALIDATION_API2_APIKEY=sk_live_xxx            # API key
export EMAIL_VALIDATION_API2_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `email-validation-api2_list` tool
...
### Call the `email-validation-api2_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `email-validation-api2_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `email-validation-api2_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `EMAIL_VALIDATION_API2_APIKEY` | API key sent with every request. |
| `EMAIL_VALIDATION_API2_BASE` | Optional override of the API base URL. |
...
```sh
./email-validation-api2-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"email-validation-api2_load","arguments":{"entity":"dns_result","query":{"id":1}}}}'
```
