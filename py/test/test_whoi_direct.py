...
class TestWhoiDirect:

    def test_should_direct_list_whoi(self):
        setup = _whoi_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-whoi", "live" if setup["live"] else "unit")
        if _skip:
...
def _whoi_direct_setup(mockres):
