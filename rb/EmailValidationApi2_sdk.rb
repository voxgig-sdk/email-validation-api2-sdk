...
require_relative 'EmailValidationApi2_types'
...
class EmailValidationApi2SDK
  ...
  def initialize(options = {})
    ...
    feature_opts = EmailValidationApi2Helpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = EmailValidationApi2Helpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, EmailValidationApi2Features.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    EmailValidationApi2Utility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = EmailValidationApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = EmailValidationApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = EmailValidationApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
    ...
    ctx.spec = EmailValidationApi2Spec.new({
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => EmailValidationApi2Error.new(
        "#{op}_allow",
        "EmailValidationApi2SDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue EmailValidationApi2Error => err
    ...
    if fetched.is_a?(Hash)
      status = EmailValidationApi2Helpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = EmailValidationApi2Error.new(
        "graphql_error", "EmailValidationApi2SDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.DnsResult.list / client.DnsResult.load({ "id" => ... })
  def DnsResult(data = nil)
    require_relative 'entity/dns_result_entity'
    DnsResultEntity.new(self, data)
  end
  ...
  # Canonical facade: client.EmailValidate.list / client.EmailValidate.load({ "id" => ... })
  def EmailValidate(data = nil)
    require_relative 'entity/email_validate_entity'
    EmailValidateEntity.new(self, data)
  end
  ...
  # Canonical facade: client.Grammar.list / client.Grammar.load({ "id" => ... })
  def Grammar(data = nil)
    require_relative 'entity/grammar_entity'
    GrammarEntity.new(self, data)
  end
  ...
  # Canonical facade: client.Redact.list / client.Redact.load({ "id" => ... })
  def Redact(data = nil)
    require_relative 'entity/redact_entity'
    RedactEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = EmailValidationApi2SDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
