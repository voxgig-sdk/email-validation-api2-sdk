...
class TestSslDirect:
    ...
    def test_should_direct_list_ssl(self):
        setup = _ssl_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-ssl", "live" if setup["live"] else "unit")
        if _skip:
