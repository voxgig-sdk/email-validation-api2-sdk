...
class EmailValidationApi2SDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = EmailValidationApi2Helpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = EmailValidationApi2Helpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!EmailValidationApi2Features::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, EmailValidationApi2Features::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return EmailValidationApi2Utility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = EmailValidationApi2Helpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = EmailValidationApi2Helpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = EmailValidationApi2Helpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new EmailValidationApi2Error($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = EmailValidationApi2Helpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new EmailValidationApi2Error("graphql_error",
                "EmailValidationApi2SDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Utility($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_utility_ === null) {
                $this->_utility_ = new UtilityEntity($this, null);
            }
            return $this->_utility_;
        }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new EmailValidationApi2SDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
