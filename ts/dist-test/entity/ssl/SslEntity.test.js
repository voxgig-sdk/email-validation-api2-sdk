...
(0, node_test_1.describe)('SslEntity', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when EMAIL_VALIDATION_API2_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('EMAIL_VALIDATION_API2_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.EmailValidationApi2SDK.test();
...
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/ssl/SslTestData.json');
    // TODO: file ready util needed?
    const entityDataSource = Fs.readFileSync(entityDataFile).toString('utf8');
    // TODO: need a xlang JSON parse utility in voxgig/struct with better error msgs
    const entityData = JSON.parse(entityDataSource);
    options.entity = entityData.existing;
    let client = __1.EmailValidationApi2SDK.test(options, extra);
    const struct = client.utility().struct;
    const merge = struct.merge;
    const transform = struct.transform;
    let idmap = transform(['ssl01', 'ssl02', 'ssl03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    idmap = env['EMAIL_VALIDATION_API2_TEST_SSL_ENTID'];
    const live = 'TRUE' === env.EMAIL_VALIDATION_API2_TEST_LIVE;
    const transport = (0, live_runner_1.createLiveTransport)();
    if (live) {
        const rawIds = process.env['EMAIL_VALIDATION_API2_TEST_SSL_ENTID'];
        idmap = rawIds && rawIds.trim() ? JSON.parse(rawIds) : {};
        if (!idmap || Array.isArray(idmap) || typeof idmap !== 'object') {
            throw new Error('Live ENTID must be a JSON object');
        }
        client = new __1.EmailValidationApi2SDK(merge([
...
//# sourceMappingURL=SslEntity.test.js.map